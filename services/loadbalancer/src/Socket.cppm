...
public:
    ...
    static Socket create(int domain = AF_INET, int type = SOCK_STREAM, int protocol = 0) {
        int raw_fd = ::socket(domain, type, protocol);
        if (raw_fd < 0) {
    ...
    void bind_and_listen(int port, int backlog = 65535) {
        ...
        int opt = 1;
        // SO_REUSEPORT allows scaling via multiple loadbalancer instances if needed
        if (::setsockopt(fd_.get(), SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt)) == -1) {
             throw std::system_error(errno, std::system_category(), "setsockopt SO_REUSEADDR|SO_REUSEPORT failed");
        }
    ...
    std::optional<Socket> accept_connection() noexcept {
        ...
        int client_fd = ::accept(fd_.get(), reinterpret_cast<sockaddr*>(&client_addr), &client_len);
        if (client_fd < 0) {
            return std::nullopt; // Error code is in errno, but EAGAIN/EWOULDBLOCK are expected non-error states
        }
        
        Socket client_sock{FileDescriptor{client_fd}};
        
        // Ensure the newly accepted socket is also non-blocking
        if (!set_nonblocking_flag(client_sock.get_fd())) {
             // Let the RAII destructor close the fd.
            return std::nullopt;
        }
